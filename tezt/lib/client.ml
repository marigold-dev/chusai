...
let address ?(hostname = false) ?from peer =
  ...
  | Some endpoint -> Tezt.Runner.address ~hostname ?from:(runner endpoint) (runner peer)
;;
...
let create_with
  ...
  let base_dir =
    ...
    | Some dir -> dir
  in
  { path; name; color; base_dir; additional_bootstraps = []; node = node_with_media }
;;
...
let spawn_rpc_aux
  ...
  let process =
    let data =
      Option.fold ~none:[] ~some:(fun x -> [ "with"; Tezt.JSON.encode_u x ]) data
    in
    let query_string = Option.fold ~none:"" ~some:Url.query_to_string query_string in
  ...
  let parse process =
    ...
    Lwt.return (Tezt.JSON.parse ~origin:(Url.path_to_string path ^ " response") output)
  in
...
let shell_header ?node ?chain ?block client =
  spawn_shell_header ?node ?chain ?block client |> Tezt.Process.check_and_read_stdout
;;
...
let import_signer_key ?node ?force client key signer_uri =
  spawn_import_signer_key ?node ?force client key signer_uri |> Tezt.Process.check
;;
...
let activate_protocol ?node ~protocol ?fitness ?key ?timestamp ?parameter_file client =
  spawn_activate_protocol ?node ~protocol ?fitness ?key ?timestamp ?parameter_file client
  |> Tezt.Process.check
;;
...
let spawn_bake_for
  ...
  spawn_command
    ...
    @ optional_arg
        ...
        string_of_int
        minimal_nanotez_per_gas_unit
    @ optional_arg ~name:"minimal-nanotez-per-byte" string_of_int minimal_nanotez_per_byte
...
let spawn_generate_alias ?hooks ?(force = false) name client =
  ( spawn_command
      ?hooks
      client
      ([ "bls"; "gen"; "keys"; name ] @ optional_switch "force" force)
  , name )
;;

let generate_alias ?hooks ?force name client =
  let p, alias = spawn_generate_alias ?hooks ?force name client in
  let open Lwt.Syntax in
  let* () = Tezt.Process.check p in
  Lwt.return alias
;;
...
let originate_contract
  ...
  | None ->
    Tezt.Test.fail "Cannot extract contract hash from client_output: %s" client_output
  ...
  | Some additional_account_keys ->
    ...
    let* parameter_file =
      Protocol.write_parameter_file
        ...
        ~base:(Option.fold ~none:(Either.right protocol) ~some:Either.left parameter_file)
        []
    in

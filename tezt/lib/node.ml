...
module Make (C : CONFIG) () = struct
    ...
    let () =
      Printexc.register_printer (function
          | Terminated_before { node; event; where = None } ->
            Some (Format.sprintf "%s terminated before event occurred: %s" node event)
  ...
  let handle_raw_event node line =
    ...
    | Some raw_event ->
      ...
      List.iter (fun handler -> handler raw_event) node.persistent_event_handlers;
      ...
      | Some events ->
        let rec loop acc = function
          | [] ->
            node.event_handlers
              <- Tezt.Base.String_map.add name (List.rev acc) node.event_handlers
  ...
  let run
    ...
    let event_process =
      ...
      | Some runner ->
        ...
        let process = Tezt.Process.spawn ~name ~runner ~log_output:false cmd arguments in
        Some process
    in
      ...
      let args =
        List.fold_right
          (fun (prefix, level) args ->
            Format.sprintf "section-prefix=%s:%s" prefix (Level.to_string level) :: args)
    ...
    let process =
      Tezt.Process.spawn
        ...
        arguments
    in
    let running_status = Session.{ process; session_state; event_loop = None } in
  ...
  let wait_for_full ?where node name filter =
    ...
    let current_events =
      Tezt.Base.String_map.find_opt name node.event_handlers |> Option.value ~default:[]
    in
    let new_map =
...
module Tezos = struct
  ...
  let make_argument = function
    ...
    | History_mode (Full (Some i)) -> [ "--history-mode"; "full:" ^ string_of_int i ]
    ...
    | Synchronisation_threshold x -> [ "--synchronisation-threshold"; string_of_int x ]
  ...
  module Conf = struct
    ...
    let base_name = "node"
    let default_colors = Tezt.Log.Color.[| FG.cyan; FG.magenta; FG.yellow; FG.green |]
  end
  ...
  let check_error ?exit_code ?msg node =
    ...
    | N.Running { process; _ } -> Tezt.Process.check_error ?exit_code ?msg process
  ;;

  let wait node =
    match node.N.status with
    | N.Not_running ->
      Tezt.Test.fail "node %s is not running, cannot wait for it to terminate" (name node)
  ...
  let config_init node arguments = spawn_config_init node arguments |> Tezt.Process.check
  ...
  let update_level node current_level =
    ...
    | N.Running status ->
      ...
      | Some old_level -> status.session_state.level <- Some (max old_level current_level)));
  ...
  let update_identity node identity =
    ...
    | Running status ->
      ...
      List.iter (fun resolver -> Lwt.wakeup_later resolver (Some identity)) pending
  ;;
  ...
  let handle_event node N.Event.{ name; value } =
    ...
    | "node_chain_validator.v0" ->
      ...
      | Some [ _timestamp; details ] ->
        (match
           Tezt.JSON.(details |-> "event" |-> "processed_block" |-> "level" |> as_int_opt)
         with
        | None -> ()
  ...
  let check_event ?where node name promise =
    ...
    match result with
    | None ->
      raise (N.Event.Terminated_before { node = node.N.name; event = name; where })
    | Some x -> Lwt.return x
  ;;
  ...
  let run ?on_terminate ?event_level ?event_sections_levels node arguments =
    ...
    do_run_command ?on_terminate ?event_level ?event_sections_levels node arguments
  ;;
  ...
  module Config_file = struct
      ...
      `O
        [ ( "genesis"
          , `O
              ...
              ; "block", `String "BLockGenesisGenesisGenesisGenesisGenesisf79b5d1CoW2"
              ; "protocol", `String Protocol.genesis_hash
              ] )
        ; ( "genesis_parameters"
          , `O [ "values", `O [ "genesis_pubkey", `String Key.activator.public_key ] ] )
    ...
    let set_sandbox_network_with_user_activated_upgrades upgrade_points old_config =
      let network =
        ...
        |> Tezt.JSON.annotate ~origin:"set_sandbox_network_with_user_activated_upgrades"
        |> Tezt.JSON.put
    ...
    let set_sandbox_network_with_user_activated_overrides overrides old_config =
      let network =
        sandbox_network_config
        |> Tezt.JSON.annotate ~origin:"set_sandbox_network_with_user_activated_overrides"
        |> Tezt.JSON.put
    ...
    let set_prevalidator
      ...
      let prevalidator =
        `O
          ...
          ; "max_refused_operations", `Float (float_of_int max_refused_operations)

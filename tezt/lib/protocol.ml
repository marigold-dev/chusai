...
let iter_on_supported_protocols ~title ~protocols ?(supports = Any_protocol) f =
  ...
  | [] ->
    failwith
      (Format.asprintf
         "test %s was registered with ~protocols:[%s] %s, which results in an empty list \
          of protocols"
         title
...
let register_regression_test ~__FILE__ ~title ~tags ?supports body protocols =
  let wrapped_test protocol =
    let title, tags = add_to_test_parameters protocol title tags in
    Tezt.Regression.register ~__FILE__ ~title ~tags (fun () -> body protocol)
  in
  iter_on_supported_protocols ~title ~protocols ?supports wrapped_test
;;

let write_parameter_file ?(additional_bootstrap_accounts = []) ~base parameter_overrides =
      ...
      List.map
        (fun (account, default_balance) ->
          `A
            ...
            ; `String
                (string_of_int (Option.value ~default:4000000000000 default_balance))
            ])

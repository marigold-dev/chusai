...
let run_main_redeem_xtz_test 
    ...
    (run_assertions : Atom.status -> main_redeem_test_props -> Atom.status) : Atom.status = 
...
let _test_Wallet_sc_redeem_xtz_with_ticket () =
  ...
  run_main_redeem_xtz_test 
    ...
    (fun (result:Atom.status) ({wallet_storage; wallet_balance; mint_balance} : main_redeem_test_props) -> 
      let {mint_address; bridge_address; ticket_storage} = wallet_storage in
      Atom.and_list
      [ Atom.assert_is_ok result ""
      ; Atom.assert_equals ticket_storage (None : chusai_ticket_storage) "" 
      ; Atom.assert_equals wallet_balance  (ticket_amount * 1tez)  ""
      ; Atom.assert_equals mint_balance 0tez ""
      ])
...
let _test_Wallet_sc_redeem_xtz_with_storage_None () =
  ...
  run_main_redeem_xtz_test 
    ...
    (fun (contr: wallet_parameter contract) ->
      Atom.transfer_to_contract_ contr Redeem_xtz 0tez)
    (fun (result:Atom.status) ({wallet_storage; wallet_balance; mint_balance} : main_redeem_test_props) -> 
      Atom.and_list 
      [ Atom.assert_rejected_with_error result (Test.compile_value "wallet_sc:No ticket found in storage") "Assertion failed: should have been rejected"
      ; Atom.assert_equals wallet_balance  0tez  ""
      ; Atom.assert_equals mint_balance (ticket_amount * 1tez)  ""
      ])


let suite = Atom.make_suite
"Wallet_sc: Test suite for Redeem_xtz endpoint"
[ Atom.make_test "Redeem ok" "redeem a simple ticket"  _test_Wallet_sc_redeem_xtz_with_ticket  
; Atom.make_test "Redeem fail" "Redeem_xtz fails if no ticket" _test_Wallet_sc_redeem_xtz_with_storage_None
]
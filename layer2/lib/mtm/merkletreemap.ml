...
module MerkleTreeMap (Hash : Hash_algo.Hash) : MerkleMap = struct
  ...
  let update_map
    ...
    let rec update_opt_node (current_node_opt : ('k, 'v) hnode option)
      ...
      =
      current_node_opt
      |> OptionExt.fold_lazy ~none:update_node_not_found ~some:update_non_empty_node
    ...
    and update_non_empty_node (current_node : ('k, 'v) hnode)
      ...
      then update_existing_node current_node
    ...
    and update_existing_node (current_node : ('k, 'v) hnode)
        : ('k, 'v) op * 'k proof * ('k, 'v) hnode option
      =
    ...
    and update_right_subtree (current_node : ('k, 'v) hnode)
        : ('k, 'v) op * 'k proof * ('k, 'v) hnode option
      =
      let op, update_right_proof, new_right_node =
    ...
    and update_left_subtree (current_node : ('k, 'v) hnode)
        : ('k, 'v) op * 'k proof * ('k, 'v) hnode option
      =
      let op, update_left_proof, new_left_node =

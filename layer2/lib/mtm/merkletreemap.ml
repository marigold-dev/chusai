...
module MerkleTreeMap(Hash : Hash_algo.Hash)  : MerkleMap = 
  struct
    ...
    module Op = struct
      ...
      let matches_proof (op : ('k, 'v) t) (proof : 'k Proof.t) : bool = 
        match op with
        | Noop noop -> 
    ...
    *)
    let update_map (key : 'k) (updater : 'v option -> 'v option) (MerkleTreeMap mtm : ('k, 'v) t) : ('k, 'v) op * 'k proof * ('k, 'v) t =
      ...
      let rec remove_node (current_node : ('k, 'v) hnode) : ('k, 'v) op * 'k proof * ('k, 'v) hnode option =
        let proof, node = 
          ...
          | Some original_left, Some right -> 
            ...
            p, Some n
          in
          Op.Remove {key = current_node.content.key}, proof, node
      ...
      let replace_value (current_node : ('k, 'v) hnode) (new_value : 'v) : ('k, 'v) op * 'k proof * ('k, 'v) hnode option =
          ...
          let op = Op.Update {key = current_node.content.key; old_value = current_node.content.value; new_value = new_value} in 
          op, p, Some n
      in
      ...
      let rec update_node (current_node_opt : ('k, 'v) hnode option) : ('k, 'v) op * 'k proof * ('k, 'v) hnode option =
        ...
        |> Option.fold
          ~none:(
            updater None
            |> Optionext.fold_lazy 
              (fun () -> Op.Noop {key = key}, Proof.NoChange H.tempty, None)
              insert)
      ...
      let op, proof, new_root = update_node mtm in

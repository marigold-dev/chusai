...
module MerkleTreeMap(Hash : Hash_algo.Hash) : MerkleMap = 
  struct
    ...
    module Proof = struct
      ...
      let matches_hashes (proof : 'k t) (initial_root_hash : hash) (final_root_hash : hash) : bool =
        let rec compute_hashes (current_proof : 'k t) : thash * thash =
          ...
          | NoChange h -> h, h (* no changes so intial and final hashes are the same *)
            in
          initial, final
      ...
      let matches_op (proof : 'k t) (op : ('k, 'v) op) : bool = 
        ...
        | Noop noop -> 
          let rec is_valid_noop = function
           ...
           | GoRight go_right -> go_right.key < noop.key && is_valid_noop go_right.right_proof
           | _ -> false
        in 
        is_valid_noop proof
        | Insert insert_op -> 
          let rec is_valid_insert = function
            | GoLeft go_left -> go_left.key > insert_op.key && is_valid_insert go_left.left_proof
            | GoRight go_right -> go_right.key < insert_op.key && is_valid_insert go_right.right_proof
            | NewLeaf new_leaf -> new_leaf.key = insert_op.key && new_leaf.vhash = (H.value insert_op.value)
            | _ -> false
          in
          is_valid_insert proof
        | Update update_op ->
          let rec is_valid_update = function
          | GoLeft go_left -> go_left.key > update_op.key && is_valid_update go_left.left_proof
          | GoRight go_right -> go_right.key < update_op.key && is_valid_update go_right.right_proof
        ...
        | Remove remove_op -> 
          ...
          | GoLeft go_left -> go_left.key > remove_op.key && is_valid_remove go_left.left_proof
          | GoRight go_right -> go_right.key < remove_op.key && is_valid_remove go_right.right_proof
          | Remove remove_proof ->
    ...
    let rec extract_biggest_child (current_node : ('k, 'v) hnode) : 'k proof * 'k * 'v * ('k, 'v) hnode option =
      ...
      | None, None -> 
        let proof = Proof.Remove(Proof.RemoveLeaf {key = current_node.content.key; vhash = current_node.content.vhash}) in 
        proof, current_node.content.key, current_node.content.value, None 
      | Some left, None ->
        let proof = Proof.Remove (Proof.ReplaceWithLeftChild {
    ...
    let remove_node (current_node : ('k, 'v) hnode) : ('k, 'v) op * 'k proof * ('k, 'v) hnode option =
      let proof, node = 
        match current_node.content.left, current_node.content.right with
        | None, None -> 
          let p = Proof.Remove(Proof.RemoveLeaf {vhash = current_node.content.vhash; key = current_node.content.key}) in
          p, None
        | None, Some right_child -> 
          let p = Proof.Remove (Proof.ReplaceWithRightChild {
            initial_key = current_node.content.key;
            initial_vhash = current_node.content.vhash;
        ...
        | Some left_child, None -> 
          let p = Proof.Remove (Proof.ReplaceWithLeftChild {
            initial_key = current_node.content.key;
            initial_vhash = current_node.content.vhash;
        ...
        | Some original_left, Some right -> 
          ...
          let p = Proof.Remove (Proof.ReplaceWithBiggestLeft {
            initial_key = current_node.content.key;
            initial_vhash = current_node.content.vhash;
    ...
    *)
    let update_map (key : 'k) (updater : 'v option -> 'v option) (MerkleTreeMap root : ('k, 'v) t) : ('k, 'v) op * 'k proof * ('k, 'v) t =
      ...
      let op, proof, new_root = update_opt_node root in
    ...
    let show (kfmt : Format.formatter -> 'k -> unit) (vfmt : Format.formatter -> 'v -> unit) (MerkleTreeMap t : ('k, 'v) t): string =
      String.concat "" [
        "MerkleTreeMap (";

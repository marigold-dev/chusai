...
let test_empty () =
  check_lists [] (Mtm.to_list Mtm.empty)
...
let test_from_one_elem_list () = 
  check_lists 
    [("a", 1)]
    (Mtm.to_list @@ Mtm.from_list @@ [("a", 1)])

let test_from_two_elem_list () = 
  check_lists 
  [("a", 1); ("b", 2)]
  (Mtm.to_list @@ Mtm.from_list @@ [("a", 1); ("b", 2)])
...
let test_remove_from_empty () = 
  check_lists  
    [] Mtm.(to_list @@ Tools.third @@ Mtm.remove "non-existent" Mtm.empty)
...
let test_remove_leaf () = 
  check_lists  
    [] (Mtm.to_list @@ Tools.third @@ Mtm.remove "a" @@ Mtm.from_list [("a", 1)])
...
let test_remove_node_right_right () = 
  check_lists  
    [("b", 2); ("c", 3)] (Mtm.to_list @@ Tools.third @@ Mtm.remove "a" @@ Mtm.from_list [("a", 1); ("b", 2); ("c", 3)])
...
let test_remove_2 () = 
  check_lists  
    [("a", 1); ("c", 3); ("d", 4)] Mtm.(to_list @@ Tools.third @@ Mtm.remove "b" @@ Mtm.from_list [("b", 2); ("d", 4); ("c", 3); ("a", 1)])

let test_remove_node_right_left_left_left () = 
  check_lists  
    [("b", 2); ("c", 3); ("d", 4); ("e", 5)] Mtm.(to_list @@ Tools.third @@ Mtm.remove "a" @@ Mtm.from_list [("a", 1); ("e", 5); ("d", 4); ("c", 3); ("b", 2)])

let test_remove_node_right_left_y () = 
  check_lists  
    [("b", 2); ("c", 3); ("d", 4); ("e", 5)] Mtm.(to_list @@ Tools.third @@ Mtm.remove "a" @@ Mtm.from_list [("a", 1); ("e", 5); ("d", 4); ("b", 2); ("c", 3)])

    
let test_remove_node_left () = 
  check_lists  
    [("a", 1)] Mtm.(to_list @@ Tools.third @@ Mtm.remove "b" @@ Mtm.from_list [("b", 2); ("a", 1)])

let test_remove_node_left_left () = 
  check_lists  
    [("a", 1); ("b", 2)] Mtm.(to_list @@ Tools.third @@ Mtm.remove "c" @@ Mtm.from_list [("c", 3); ("b", 2); ("a", 1)])

let test_remove_y_node () = 
  check_lists  
    [("a", 1); ("c", 3)] Mtm.(to_list @@ Tools.third @@ Mtm.remove "b" @@ Mtm.from_list [("a", 1); ("b", 2); ("c", 3)])
...
let test_verify_proof_update_one_elem_list () =
  ...
  let test_verify_invalid_proof_insert () =
    ...
    let _ = check_true @@ Mtm.verify_proof op_ab proof_ab (Mtm.root_hash map_a) (Mtm.root_hash map_ab) in
    let _ = check_true @@ Mtm.verify_proof op_bc proof_bc (Mtm.root_hash map_c) (Mtm.root_hash map_bc) in
    let _ = check_false @@ Mtm.verify_proof op_bc proof_bc (Mtm.root_hash map_c) (Mtm.root_hash map_bc) in
    ()
let test_verify_remove_from_empty_map () =
  let op, proof, final_map = Mtm.remove "a" Mtm.empty in

  let _ = check_true @@ Mtm.verify_proof op proof (Mtm.root_hash Mtm.empty) (Mtm.root_hash final_map) in
  ()
...
let test_verify_interchange_remove_proofs () =
  ...
  let _ = check_true @@ Mtm.verify_proof op_a  proof_a (Mtm.root_hash map_a) (Mtm.root_hash final_map_a) in
  let _ = check_false @@ Mtm.verify_proof op_c proof_c (Mtm.root_hash map_a) (Mtm.root_hash final_map_a) in
  let _ = check_true @@ Mtm.verify_proof op_c proof_c (Mtm.root_hash map_c) (Mtm.root_hash final_map_c) in
  let _ = check_false @@ Mtm.verify_proof op_a proof_a (Mtm.root_hash map_a) (Mtm.root_hash final_map_c) in
  ()

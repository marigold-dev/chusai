...
let test_pack_chusai_contract () =
  ...
  >>=? fun result ->
  (* see gen_test_data_for_add.mligo to generate expected result *)
...
let test_exec_chusai_contract () =
  let script_expr = from_file "./contract/add.tz" in

  let from_string = Tezos_013_PtJakart_test_helpers.Expr.from_string in
  (* see gen_test_data_for_add.mligo to generate expected result *)
  let storage_expr = from_string "Some 0x050001" in
  let arg_expr = from_string "0x050002" in

  let open Tezos_protocol_013_PtJakart.Protocol.Alpha_context in
  execute ~script_expr ~storage_expr ~arg_expr ~entrypoint:Entrypoint.default
  >>=? fun result ->
  (* see gen_test_data_for_add.mligo to generate expected result *)
  let expected = of_bytes "0505090a00000003050003" in
  Alcotest.(check bytes) "didn't match" result expected |> return
...
let () =
  ...
  Alcotest_lwt.run "Utils" [
    "Chusai Contract",
    [ tztest "test pack chusai contract" `Quick test_pack_chusai_contract
    ; tztest "test exec chusai contract" `Quick test_exec_chusai_contract
    ]
  ] |> Lwt_main.run
